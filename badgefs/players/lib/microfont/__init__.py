...
class MicroFont:
    def __init__(self, filename, cache_index=False, cache_chars=False):
        ...
        magic, height, baseline, max_width, monospaced, index_len = struct.unpack("<4sBBBBL", header_data)
        if magic != b"MFNT":
        ...
        for y in range(ch_height):
            for x in range(ch_width):
                ...
                for step in range(2):
                    s = (step << 4) + (step << 3)
                    dx = dst_x + (
                        ((((x + off_x) << 6) + s) * cos_a - (((y + off_y) << 6) + s) * sin_a + (1 << 11)) >> 12
                    )
                    dy = dst_y + (
                        ((((x + off_x) << 6) + s) * sin_a + (((y + off_y) << 6) + s) * cos_a + (1 << 11)) >> 12
                    )
